(function (global, factory) {
	...
	function spawnMessageValid(message) {
	...
	function appMessageValid(message) {
	  return messageValid(message) && message.viaHub && ['CONNACK', 'PUBLISH', 'PING'].includes(message.type);
	}
	/**
	...
	function hubMessageValid(message) {
	  return messageValid(message) && !message.viaHub && ['CONNECT', 'PUBLISH', 'PONG', 'SPAWN'].includes(message.type);
	}
	/**
	...
	function hub(chrome) {
	  ...
	  var appIdByWindow = chrome.appIdByWindow,
	      windowByAppId = chrome.windowByAppId,
	      appTimeout = chrome.appTimeout,
	      ...
	      case 'PUBLISH':
	        {
	          if (!publishMessageValid(message)) {
	            console.warn('Message incomplete for a PUBLISH command!\n' + JSON.stringify(message));
	            return;
	          }
	          /**
	          ...
	          debug('Routing %o from %o to %o - %O', 'PUBLISH', message.source, message.target, message);
