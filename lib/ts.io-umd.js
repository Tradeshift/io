(function (global, factory) {
  ...
  function hub(chrome) {
    ...
    if (chrome.appIdByWindow(window.top) !== CHROME_APP_ID) {
      throw new Error("Can't initialize ts.io() Hub. Expected appIdByWindow(window.top) to be '".concat(CHROME_APP_ID, "', received '").concat(chrome.appIdByWindow(window.top), "' instead."));
    }

    if (chrome.windowByAppId(CHROME_APP_ID) !== window.top) {
      throw new Error("Can't initialize ts.io() Hub. Expected windowByAppId('".concat(CHROME_APP_ID, "') to be window.top, received '").concat(chrome.windowByAppId(CHROME_APP_ID), "' instead."));
    }
    ...
    window.addEventListener('message', function (event) {
      ...
      if (!appWindows.has(event.source)) {
        ...
        if (message.type === 'CONNECT') {
          var appId = chrome.appIdByWindow(event.source);
      ...
      switch (message.type) {
        case 'PUBLISH':
          ...
          var targetWindow = chrome.windowByAppId(message.target, event.source);
