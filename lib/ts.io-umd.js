(function(global, factory) {
	...
	var _iterDefine = function(Base, NAME, Constructor, next, DEFAULT, IS_SET, FORCED) {
		...
		if ($anyNative) {
			...
			if (IteratorPrototype !== Object.prototype && IteratorPrototype.next) {
				...
				if (!_library && typeof IteratorPrototype[ITERATOR$3] != 'function')
		...
		function Log(namespace, color) {
			...
			this.log = function(message) {
				...
				(_console = console).log.apply(
					...
					[
						'%c'
							.concat(_this.namespace, '%c - ')
							.concat(message, ' - %c')
							.concat(parseFloat(deltaTime).toFixed(2), 'ms'),
						'color: '.concat(_this.color, ';'),
						'font-weight: normal;'
					].concat(optionalParams, ['color: '.concat(_this.color, ';')])
				);
			};
		};
	/**
	...
	function flushQueue(token) {
		if (messageQueue.length) {
			messageQueue.reverse().forEach(function(queuedMessage) {
				return queuedMessage.targetWindow.postMessage(
					_objectSpread({}, queuedMessage.message, {
						token: token
					}),
					targetOrigin
				);
	...
	function appMessageValid(message) {
		return (
			...
			message.viaHub &&
			['CONNACK', 'PUBLISH', 'PING', 'SPAWN-SUCCESS', 'SPAWN-FAIL'].includes(message.type)
		);
	}
	/**
	...
	function hubMessageValid(message) {
		return (
			messageValid(message) &&
			!message.viaHub &&
			['CONNECT', 'PUBLISH', 'PONG', 'SPAWN', 'SPAWN-SUCCESS', 'SPAWN-FAIL'].includes(message.type)
		);
	}
	/**
	...
	function hub(chrome) {
		...
		var _chrome$appByWindow = chrome.appByWindow,
			appByWindow =
				_chrome$appByWindow === void 0 ? invalidFunction('appByWindow') : _chrome$appByWindow,
			_chrome$windowByApp = chrome.windowByApp,
			windowByApp =
				_chrome$windowByApp === void 0 ? invalidFunction('windowByApp') : _chrome$windowByApp;
		/**
...

		{
			...
			if (testNotWindow) {
				throw new Error(
					"Can't initialize ts.io() Hub. Expected windowByApp('".concat(
						CHROME_APP_ID,
						"') to return a 'Window' object."
					)
				);
			} else if (testNotAppId) {
				throw new Error(
					"Can't initialize ts.io() Hub. Expected appByWindow(windowByApp('"
						.concat(CHROME_APP_ID, "')) to return '")
						.concat(CHROME_APP_ID, "'.")
				);
			}
		}
		...
		window.addEventListener('message', function(event) {
			...
			switch (message.type) {
				...
				case 'PONG':
					appPongs.set(
						token,
						_objectSpread({}, appPongs.get(token), {
							lastPong: window.performance.now()
						})
					);
					break;
