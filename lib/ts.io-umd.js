(function (global, factory) {
	...
	function appMessageValid(message) {
	  return messageValid(message) && message.viaHub && ['CONNACK', 'PUBLISH', 'PING', 'SPAWN-SUCCESS', 'SPAWN-FAIL'].includes(message.type);
	}
	/**
	...
	function hubMessageValid(message) {
	  return messageValid(message) && !message.viaHub && ['CONNECT', 'PUBLISH', 'PONG', 'SPAWN', 'SPAWN-SUCCESS', 'SPAWN-FAIL'].includes(message.type);
	}
	/**
	...
	function app() {
	  ...
	  appInstance = {
	    ...
	    on: function on(topic, handler) {
	      ...
	      return function () {
	        return _this.off(topic, handler);
	      };
	    },
	     ...
	     * @returns {Function} Deregistrator of listener.
	     */
	    once: function once(topic, handler) {
	      var _this2 = this;

	      var wrappedHandler = function wrappedHandler(message) {
	        _this2.off(topic, wrappedHandler);
	      ...
	      return function () {
	        return _this2.off(topic, wrappedHandler);
	      };
	    },
	     ...
	     */
	    off: function off(topic, handler) {
	      ...
	      debug('%s handler %o - %O', deleted ? 'Deleted' : "Didn't find", topic, handler);
	      return deleted;
	    },
	     ...
	     * @param {*=} data Data.
	     */
	    emit: function emit(topic, target) {
	    ...
	    add: function add(method, handler) {
	    ...
	    call: function () {
	      ...
	      /*#__PURE__*/
	      regenerator.mark(function _callee(method, target) {
	        var data,
	            message,
	            _args = arguments;
	          ...
	          while (1) {
	            switch (_context.prev = _context.next) {
	              case 0:
	                ...
	                _context.t0 = method;
	                _context.next = _context.t0 === 'spawn' ? 4 : 7;
	                break;
	  ...
	  function eventHandler(event) {
	    ...
	    if (message.type === 'CONNACK') {
	      ...
	      if (message.source) {
	        ...
	        if (methodHandlers.has('spawn')) {
	           ...
	           */
	          methodHandlers.get('spawn').apply({}, [message.data, function resolve(data) {
	            postMessage$1({
	              type: 'SPAWN-SUCCESS',
	          ...
	          postMessage$1({
	            ...
	            target: message.source,
	            topic: message.topic,
	            data: message.target + " doesn't have a 'spawn' handler, it's not compatible with this SPAWN request.",
	            token: token
	          });
	        }
	      }

	      var queueLength = flushQueue(token);

	      if (queueLength) {
	        debug('Publishing %s queued message%s', queueLength, queueLength === 1 ? '' : 's');
	      }
	    ...
	    if (message.type !== 'PING') {
	      debug('Received %s (%o) from %o - %O', message.type, message.topic, message.source, message);
	    }
	...
	function hub(chrome) {
	  ...
	  var _chrome$appByWindow = chrome.appByWindow,
	      appByWindow = _chrome$appByWindow === void 0 ? invalidFunction('appByWindow') : _chrome$appByWindow,
	      _chrome$windowByApp = chrome.windowByApp,
	      windowByApp = _chrome$windowByApp === void 0 ? invalidFunction('windowByApp') : _chrome$windowByApp;
	  /**
	   * Quickly test that appByWindow & windowByApp work for 'Tradeshift.Chrome'
	   */

	  {
	    var testChromeWindow = windowByApp(CHROME_APP_ID);
	    var testNotWindow = !(testChromeWindow instanceof Window);
	    var testNotAppId = appByWindow(testChromeWindow) !== CHROME_APP_ID;

	    if (testNotWindow) {
	      throw new Error("Can't initialize ts.io() Hub. Expected windowByApp('".concat(CHROME_APP_ID, "') to return a 'Window' object."));
	    } else if (testNotAppId) {
	      throw new Error("Can't initialize ts.io() Hub. Expected appByWindow(windowByApp('".concat(CHROME_APP_ID, "')) to return '").concat(CHROME_APP_ID, "'."));
	    }
	  }
	  ...
	  var add = function add(method, handler) {
	    methodHandlers.set(method, handler);
	  };

	  var call = function call(method, argsArr) {
	  ...
	  window.addEventListener('message', function (event) {
	    ...
	    if (!appWindows.has(event.source)) {
	      ...
	      if (message.type === 'CONNECT') {
	        var appId = appByWindow(event.source);
	      ...
	      case 'SPAWN-FAIL':
	        {
	          ...
	          var targetWindow = windowByApp(message.target, event.source);
	          ...
	          if (message.type.indexOf('SPAWN') === 0) {
	            call('spawn.submit', Object.values({
	              app: message.source,
	      ...
	      case 'SPAWN':
	        {
	          ...
	          try {
	            var _appId = call('spawn', Object.values({
	              app: message.target,
