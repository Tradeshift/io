(function (global, factory) {
  ...
  (global.ts = global.ts || {}, global.ts.app = factory());
  ...
  function debugEnabled(namespace) {
    ...
    try {
      ...
      /* DATA_CLONE_ERR */
          ...
          console.warn('ts.app error while setting up debug logging. You should ignore this message or set %o while sandboxing your iframe. %O', 'allow-same-origin', error);
        } else {
  ...
  function postMessage(message, targetWindow) {
    ...
    try {
      ...
      /* DATA_CLONE_ERR */
          ...
          throw new Error("ts.app.publish called with { data } argument that can't be cloned using the structural clone algorithm.");
        } else {
    ...
    window.addEventListener('message', function (event) {
      ...
      if (!appWindows.has(event.source)) {
        ...
        if (message.type === 'CONNECT') {
          var _appId = chrome.appIdByWindow(event.source);

          var _token = uuid_1();

          debug('CONNECT %o', _appId);
      ...
      var appId = message.source = appWindow.appId;
...
//# sourceMappingURL=ts.app-umd.js.map
