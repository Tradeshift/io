...
function debugEnabled(namespace) {
	...
	try {
		...
		if (
			...
			(error.name === 'DataCloneError' || error.code === 25) /* DATA_CLONE_ERR */
		) {
			console.warn(
...
class Log {
	...
	constructor(namespace, color) {
		...
		this.log = (message, ...optionalParams) => {
			...
			console.log(
				`%c${this.namespace}%c - ${message} - %c${parseFloat(deltaTime).toFixed(2)}ms`,
				`color: ${this.color};`,
				'font-weight: normal;',
				...optionalParams,
				`color: ${this.color};`
	...
	if (messageQueue.length) {
		messageQueue
			...
			.forEach(queuedMessage =>
				queuedMessage.targetWindow.postMessage({ ...queuedMessage.message, token }, targetOrigin)
			);
		return true;
	}
	return false;
		...
		switch (message.type) {
			case 'PUBLISH':
				handlersByTopic.forEach(
					(handlers, topic) =>
						matchTopic(topic, message.topic) && handlers.forEach(handler => handler(message))
				);
				break;
			case 'PING':
...
function hub(chrome) {
	...
	add('kill', function(targetWindow) {
		try {
			...
			appPongs.get(token).timeoutIds.forEach(timeoutId => clearTimeout(timeoutId));
	...
	*/
	function pingApp(opts) {
		...
		if (now - lastPong < 3 * HEARTBEAT) {
			...
			postMessage({ type: 'PING', viaHub: true, target: appId, token }, targetWindow);
		} else {
