(function (global, factory) {
	...
	var TOPIC_AFTER_CONNECT = 'after-connect';
	...
	var hub;
	var windowMapper = {
	  appByWindow: function appByWindow(win) {
	  ...
	  windowByApp: function windowByApp(app, sourceWindow) {
	...
	var createIframe = function createIframe(_ref) {
	  var id = _ref.id,
	      src = _ref.src,
	...
	var killAllApps = function killAllApps() {
	...
	var createApp = function createApp(app) {
	  ...
	  var src = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'app.html';
	  apps.push(app);
	  return createIframe(_objectSpread({
	    id: app,
	    src: src
	  }, opts));
	};
	var getHub = function getHub() {
	  try {
	...
	var isWindow = function isWindow(win) {
	  return win && win.postMessage && win.frames;
	};

	var verifyTestMessage = function verifyTestMessage(message, source) {
	  return source && message && message.ioTest && message.data;
	};
	var sendCommand = function sendCommand(win, command) {
	  if (isWindow(win)) {
	    win.postMessage(_objectSpread({
	      ioTest: true
	    }, command), '*');
	  } else {
	...
	var removeAppOrFail = function removeAppOrFail(app, apps, message) {
	  var idxToRemove = apps.indexOf(app);
	...
	try {
	  describe('ts.io', function () {
	    ...
	    beforeEach(function () {
	      apps = ['Tradeshift.Unsecure', 'Tradeshift.Sandbox', 'Tradeshift.Crossdomain', 'Tradeshift.SandboxCrossdomain'];
	      createApp(apps[0]);
	      createApp(apps[1], {
	        sandbox: true
	      });
	    ...
	    it('app.emit() before CONNECT', function (done) {
	      var messageCount = 0;
	      topApp.on(TOPIC_BEFORE_CONNECT, function (message) {
	    ...
	    it('app.emit() after CONNECT', function (done) {
	      var connectCount = 0;

	      var connectListener = function connectListener(event) {
	        ...
	        if (message.data.method === 'connect') {
	          delay(function () {
	            return sendCommand(event.source, {
	              command: 'emit',
	      ...
	      var messageCount = 0;
	      topApp.on(TOPIC_AFTER_CONNECT, function (message) {
	    ...
	    it('app.once() [on() + off()]', function (done) {
	      var connectCount = 0;
	      ...
	      var messageHandler = function messageHandler(message) {
	        removeAppOrFail(message.source, apps, 'Message handled by once(), more than once!');
	      ...
	      var messageCount = 0;
	      topApp.on('*', function (msg) {
	        if (++messageCount === 12) {
	          done();
	        }
	      });
	    });
	    it("hub.add('timeout')", function (done) {
	      var timeoutApp, timeoutWindow;
	      var connectCount = 0;
	      ...
	      window.addEventListener('message', connectListener);
	      hub$1.add('timeout', function (win, app) {
	    ...
	    it('app.spawn() > App not found', function (done) {
	      var connectCount = 0;
	      ...
	      var connectListener = function connectListener(event) {
	        ...
	        if (message.data.method === 'connect') {
	          ...
	          delay(function () {
	            return sendCommand(event.source, command);
	          });
	      ...
	      var spawnListener = function spawnListener(event) {
	        var message = event.data;
...
//# sourceMappingURL=ts.io.spec.js.map
